...
def test(model_name):
    ...
    test_output_file = open("./test_log/model_test_output_log.csv", "a", newline="")

    # 임의의 모델이 어떤 비디오에 대해 어떻게 예측했는지 확인하기 위한 파일
    model_log_file = open(
        "./test_log/model_log/{}_{}frame.csv".format(model_name, seq_length),
        "w",
        newline="",
    )
    ...
    testset_20frame = np.loadtxt(testset_file_20frame, delimiter=",", dtype=np.float32)
    testset_file_30frame = "/Users/jaejoon/LGuplus/main_project/lstm/dataset/mydataset_v3_test_30frames.csv"
    testset_30frame = np.loadtxt(testset_file_30frame, delimiter=",", dtype=np.float32)
    ...
    np.savetxt(
        "./test_log/confusion_matrix/{}_{}frame_confusion_matrix.csv".format(
            model_name, seq_length
        ),
        confusion_matrix,
        delimiter=",",
        fmt="%.5f",
    ...
    for i in range(output_dim):
        class_accuracy[i] = confusion_matrix[i, i] / np.sum(
            confusion_matrix[i]
        )  # 클래스 별 accuracy
